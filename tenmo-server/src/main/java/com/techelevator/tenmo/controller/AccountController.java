...
public class AccountController {
    ...
    public void pendingTransferApproval (@PathVariable Integer transferId) {
        ...
        sendBucks(accountDao.getAccountByAccountId(newTransfer.getAccountFrom()).getUserId(),
                accountDao.getAccountByAccountId(newTransfer.getAccountTo()).getUserId(), newTransfer.getAmount());
    }

    @PutMapping("/rejected/{transferId}")
    public void pendingTransferRejection (@PathVariable Integer transferId) {
        Transfer newTransfer = transferDao.getPendingTransfersByTransferId(transferId);
    ...
    public boolean sendBucks(@PathVariable Integer senderUserId,
        ...
        if (sender.getBalance().compareTo(amount) >= 0) {
            ...
            if (transferDao.getTransfersByUserId(sender.getUserId()).isEmpty()) {
                transferDao.createTransferRequest(new Transfer(2, 2, sender.getAccountId(), receiver.getAccountId(), amount ));
            }
        } else {
